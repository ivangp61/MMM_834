...
DECLARE @excDate DATE = '2017-11-13';
DECLARE @excLogid INT;
DECLARE @jobId INT;

SET @jobId = (SELECT JD.JobId	FROM DBO.JobDefinition JD WHERE JD.JobName = 'MMMHC_MEM_QNXT_EHub_ExtrEHubMember.PMC');
--(SELECT JD.*	FROM DBO.JobDefinition JD WHERE JD.JobName like '%MMMHC_MEM_QNXT_EHub_ExtrQnxtMember%');
